...
//
// auth middleware: try to require the real middleware, otherwise fallback in dev
//
let requireAuth;
try {
...
let Habit, Goal, Event, SuggestionModel;
...
router.get("/suggestions", requireAuth, async (req, res) => {
  try {
    ...
    const [habits, goals, events] = await Promise.all([
      ...
      Goal ? Goal.find({ user: userId }).lean() : Promise.resolve([]),
    ...
    for (const ev of events) {
      try {
        ...
        while (cur < e) {
          ...
          if (busyByDayHour[key] && Number.isInteger(hr) && hr >= 0 && hr < 24) busyByDayHour[key][hr] = busyByDayHour[key][hr] + 1;
 ...
 * This endpoint applies a suggestion action (best-effort). It is intentionally simple:
 * - reschedule / reduce_frequency attempt to update Habit model
 * - create_microtask attempts to append to Goal.microtasks (if present)
 * - suggest_block creates an Event (focus block)
 *
 * Returns { ok: true, details } or 400/500 errors.
 */
router.post("/execute", requireAuth, async (req, res) => {
  try {
    ...
    if (action.action === "reschedule") {
      ...
      // apply update only if the habit belongs to the user
      const habit = await Habit.findOneAndUpdate({ _id: habitId, user: userId }, { $set: update }, { new: true }).lean();
    ...
    if (action.action === "create_microtask") {
      ...
      // Add microtask to goal.microtasks array (create if not present)
      goal.microtasks = goal.microtasks || [];
      goal.microtasks.push({ text: text || "New microtask (from AI)", createdAt: new Date(), done: false });
      await goal.save();
    ...
    if (action.action === "suggest_block") {
      ...
      if (!date || startHour == null || endHour == null) return res.status(400).json({ error: "date, startHour, endHour required for suggest_block" });
      if (!Event) return res.status(500).json({ error: "Event model not available on server" });
      // Build ISO strings (local timezone)
